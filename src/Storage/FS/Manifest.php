<?php
    ...
    class Manifest extends Common\Collection implements I\Manifest
    {
        ...
        public function load(): bool
        {
            ...
            try {
                $this->content = json_decode(@file_get_contents($this->path), true, 512, JSON_THROW_ON_ERROR);
            } catch (Exception $e) {
                return false;
            }
        ...
        {
            try {
                @file_put_contents($this->path, json_encode($this->content, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR));
            } catch (Exception $e) {
                throw new Storage\Exception('Can`t save manifest file: ' . $e->getMessage(), 0, $e);
            }
        }
